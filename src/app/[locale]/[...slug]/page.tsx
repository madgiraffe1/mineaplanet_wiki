...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  setRequestLocale(locale)
  const contentType = slug[0]
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.mineaplanet.online'
  ...
  if (isListPage) {
    ...
    try {
      ...
      // 如果翻译不存在，使用默认值
      const defaultTitle = `${contentType.charAt(0).toUpperCase() + contentType.slice(1)} - Mine a Planet Wiki`
      const path = `/${contentType}`

      return {
        title: defaultTitle,
        description: `Browse all ${contentType} content for Mine a Planet Wiki`,
        alternates: buildLanguageAlternates(path, locale as Locale, siteUrl),
        robots: {
          index: true,
          follow: true,
    ...
    try {
      ...
      return {
        title: `${metadata.title} - Mine a Planet Wiki`,
